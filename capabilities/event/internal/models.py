...
EventCandidate = EventCandidateDTO
EventSemantic = EventSemanticDTO
...
class EventAgentVersions(BaseModel):
    """Complete positive Agent bindings pinned by one published Event Workflow."""

    model_config = ConfigDict(extra="forbid", frozen=True)

    extractor: int = Field(alias=EVENT_EXTRACTOR_AGENT_ID, ge=1, strict=True)
    identity: int = Field(alias=EVENT_IDENTITY_AGENT_ID, ge=1, strict=True)
    signal_analyst: int = Field(alias=EVENT_SIGNAL_ANALYST_AGENT_ID, ge=1, strict=True)

    def as_mapping(self) -> dict[str, int]:
        return self.model_dump(by_alias=True)


class EventAgentExecutionRecord(BaseModel):
    """Exact published Agent version bound before one recoverable semantic operation."""
    ...
    @classmethod
    def require_event_agent(cls, value: str) -> str:
        if value not in EVENT_AGENT_IDS:
            raise ValueError("Event Agent execution uses an unknown Agent identity")
        return value


class EventAgentExecutionJournal(BaseModel):
    """Append-only audit of exact Agent versions used by one Event batch."""

    model_config = ConfigDict(extra="forbid")

    schema_version: Literal["event_agent_execution_journal.v1"] = "event_agent_execution_journal.v1"
    batch_id: str = Field(pattern=r"^[0-9a-f]{64}$")
    executions: list[EventAgentExecutionRecord]

    @model_validator(mode="after")
    def require_unique_sorted_operations(self) -> "EventAgentExecutionJournal":
        keys = [record.operation_key for record in self.executions]
        if len(keys) != len(set(keys)):
            raise ValueError("Event Agent execution operation keys must be unique")
        if keys != sorted(keys):
            raise ValueError("Event Agent executions must be ordered by operation key")
        return self
