...
报告仅展示有实际详情推导结果的内容，本规则同时适用于结构化定稿、HTML 和发布数据包。

- 地缘政治、宏观经济故事线若只有自身信号、变量综合或总结说明，没有下游宏观或产业链的实际详情推导，应从展示报告及发布包移除。不能用一句总结、空对象、占位文字或单纯非空数组冒充详情结果。
- 产业链节点须有事实或证据依据、适用范围、可解释机制及完整判断；缺乏推导依据的节点移除。链无有效节点时移除空链；概念或其他总结单元在清理后无有效详情时一并移除，不能留下空卡片。
- 有完整证据、成立条件和推导逻辑，但方向尚未明确或仍待验证的有界判断，不视为空详情。不能仅因方向 pending、置信度低或存在不确定性就删除；审阅实际推导是否成立。
- 剔除从最下层节点向上检查。同步清理 affected_refs、upstream_refs、图节点、图边及总结；若依赖被移除对象的判断无法独立成立，重新审阅或移除，不能只删引用掩盖证据缺口。不得跨越被移除节点补造连接。
- 自身信号的直接判断和全量输入审阅仍须完成；被移除内容及 Event/Signal 的限定、排除原因留在内部 audit，不把展示过滤当作未分析输入的借口。
- 先修订 JSON，再生成 HTML 和发布包；保护原始分析，保存移除清单、原因、各层前后数量及新哈希。发布前逐单元检查总结与详情一致、无空详情和悬空引用，发布后读回核对。没有合格结果时不为凑数创建内容或发布空报告。
