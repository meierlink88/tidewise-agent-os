...
1. 每个输入 Evidence 必须且只能出现一次：进入一个 Candidate、NO_EVENT 或 NEEDS_REVIEW。
...
5. Evidence 同时包含多个现实动作、核心事实冲突、无法判断分组或缺少可靠时间锚点时进入 needs_review；不得自行拆分或猜测。
...
9. 时间必须是明确 UTC ISO-8601；不确定时不要臆造。occurred_at、announced_at、effective_at 至少一项必须存在，否则进入 needs_review。
