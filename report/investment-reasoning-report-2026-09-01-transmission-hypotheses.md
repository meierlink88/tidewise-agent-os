...
remaining_topology_pending_count: 58
adaptive_inclusion_threshold: 0.40
adaptive_continuation_threshold: 0.65
adaptive_hard_max_hops: 5
adaptive_observed_max_hops: 1
adaptive_stopped_by_confidence: 23
...
methodology_version: graph-chain-node-adaptive-transmission-v1
review_status: graph_entity_signal_and_adaptive_hypothesis_verified
---

# 投研推理报告：当前事件如何从地缘政治与宏观经济传导至产业链（动态传导）
...
>
> **向下传导标记**：`跨层推理` 表示上一层结论参与解释下一层真实锚点的方向；`同源信号` 表示同一批 Event 分别在上下两层形成直接 Signal，只能用于说明共同驱动，不能把上层锚点改写成下层锚点的直接原因；`待补证` 表示机制合理但尚未锁定真实下层锚点或缺少中间变量，不进入正式方向结论。
>
> **传导机制**：从直接 Signal 节点开始逐跳扩展，路径分数 = 起点置信度 × 关系可靠性 × 变量映射可靠性 × 时间匹配度。分数达到 `0.40` 才进入报告，达到 `0.65` 才允许继续寻找下一跳；`5` 跳仅为防止异常循环的安全上限。遇到无法映射真实 Variable、链语境不明确、循环、没有未访问邻居或分数衰减时立即停止。推理假设不写回 Graphiti Signal Fact，等待订单、价格、产能或经营数据验证。关系可靠性与变量映射可靠性是本次分析判断，不是图谱事实；路径分数用于筛选和排序，不表示统计概率。
...
本节回答“地缘政治结论如何参与下层分析”。只有目标为真实图谱锚点且传导机制能够闭合时，才形成跨层推理；同一 Event 在两层分别产生直接 Signal 时，只标记为同源信号。

#### 传到宏观经济

| Path ID | 来源结论 | 目标锚点 | 传导逻辑 | 目标结果 | 关系性质 | 支持 Evidence | 置信度 | 状态 |
|---|---|---|---|---|---|---|---|---|
| GEO-X-MAC-01 | 伊朗—美以及海湾安全对抗升温；航运安全和贸易通道可用性下降 | 增长预期修正（真实 `MacroEconomic`） | 海湾冲突使霍尔木兹油运中断风险上升，增强了“能源供应持续中断”这一全球增长下行情景成立的可能性，因此对增长预期下修形成跨层解释。该宏观锚点本身另有世界银行 Event 形成的直接 Signal，地缘结论只增强其解释，不替代直接证据。 | 降温 | 推理假设（目标另有直接证据） | 来源：EVT3cf927a6, EVTec2c2162, EVTa8b7eff4；目标：EVTcaf3f60b | 中 | 已形成解释闭环；仍需连续油运、能源价格和增长预测验证 |

`加息`锚点虽有直接 Signal，但本次材料没有形成“海湾冲突 → 能源价格/通胀 → 政策利率”的完整中间变量链，因此不把地缘政治冲突写成加息的跨层原因。

#### 传到产业链

| Path ID | 来源结论 | 目标产业链 / 节点 | 传导逻辑 | 目标结果 | 关系性质 | 支持 Evidence | 置信度 | 状态 |
|---|---|---|---|---|---|---|---|---|
| GEO-X-CHN-01 | 伊朗—美以及海湾安全对抗升温；霍尔木兹通道可用性下降 | CHN-21 油品石化贸易服务产业链：`油品运输服务`、`成品油批发交付服务` | 海湾冲突压低霍尔木兹通行能力，直接延长油品运输周期并抬高运价；批发交付依赖运输服务，因此更高物流成本和更长到货时间可能继续传至批发交付环节。 | 油品运输服务分化；成品油批发交付服务降温 | 同源直接 Signal + 节点传导假设 | EVTa8b7eff4, EVT3cf927a6 | 中 | 运输节点为同源直接证据；批发交付节点为待验证的传导假设 |
...
本次两个宏观锚点均有直接 Signal，但尚未形成“宏观锚点 → 真实 IndustryChain / ChainNode”的完整证据闭环，因此本节不新增产业链方向结论。

| 候选机制 | 当前推导逻辑 | 为什么没有发布下层结论 | 状态 |
|---|---|---|---|
| 增长预期修正 DOWN → 产业需求 | 全球增长预期下修通常会压低外需和周期品需求，再影响相应产业节点的订单、价格或产能利用率。 | 当前报告缺少中国外需、工业需求和具体真实 ChainNode 的对应 Signal，无法确定影响落在哪条链、哪个节点。 | 待补证，不进入产业链结论 |
| 加息 UP → 融资与资本开支 | 政策利率上行通常会提高融资成本和折现率，进而压制资本密集型产业的扩产与投资。 | 当前报告缺少融资成本、资本开支和真实 ChainNode 之间的图谱桥接与节点 Signal，不能指定受影响产业链。 | 待补证，不进入产业链结论 |

后续若出现中国进口成本、PPI、人民币汇率、工业需求、融资成本或资本开支观测，并能落到真实 ChainNode，才将上述候选机制升级为可发布的宏观 → 产业链传导路径。
...
本层由 **43 个真实 ChainNode 的 85 条直接 Signal** 聚合出 **54 条真实 IndustryChain**。在此基础上，本版从 45 条候选路径中接受 **29 条动态传导假设**，其余 **58 条拓扑相关记录** 继续待验证。其中 6 条路径达到继续推导阈值，但目标节点已无未访问邻居，因此本次实际最大深度自然停在 1 跳，而非预先限定为 1 跳。共享节点本链语境未确定时不自动传播；另有 3 个 Signal 节点缺少产业链归属，不虚构链关系。
...
| `9f4e1d1d-6e7…` | 机器人控制计算平台 | 市场需求 UP/LOW | 升温 | 推理假设 | 人形机器人的商业化和渗透率上升，会增加整机对运动控制、感知融合和实时决策计算的需求。控制计算平台是整机的明确组成环节，因此推测其市场需求将随整机放量而上升。 | 起点证据：EVT245a1a74, EVT685aba49 | 中期–长期（传导滞后） | 低（0.60） |

- **直接 Evidence**：EVT245a1a74, EVT685aba49。
- **反证与 Gap**：上述传导为经路径评分筛选的推理假设，仍需目标节点的订单、价格、产能或经营数据验证；未被推导的相邻节点继续作为 Evidence Gap。
...
| `97b3353f-a55…` | 服务器冷板 | 市场需求 UP/LOW | 升温 | 推理假设 | 液冷服务器商业化进度上升，意味着冷板等核心散热部件的配套量会随整机交付增加。服务器冷板是液冷服务器的真实组成环节，因此推测其市场需求上升。 | 起点证据：EVT4f8f284c | 长期（传导滞后） | 低（0.60） |

- **直接 Evidence**：EVT40bb2d15, EVT3387e8c0, EVT4f8f284c。
- **反证与 Gap**：共享节点的本链语境仍待解析；上述传导为经路径评分筛选的推理假设，仍需目标节点的订单、价格、产能或经营数据验证；未被推导的相邻节点继续作为 Evidence Gap。
- **停止条件**：若后续 Event 使节点 Signal 失效、方向反转，或链语境确认不属于本链，则关闭或修正本链结论。
...
| `d0cb4396-d6c…` | 油气集输与初步处理服务 | 产能利用率 DOWN/LOW | 降温 | 推理假设 | 油气开采供给下降会减少进入集输和初处理环节的原料量。该服务是开采产出后的明确下游环节，因此推测其产能利用率可能下降。 | 起点证据：EVTe8d46974 | 短期–中期（传导滞后） | 低（0.54） |
...
| `0091def5-b9c…` | 算力供给 | 产能利用率 UP/MEDIUM；商业化进度 UP/LOW；市场供给 UP/MEDIUM；性价比 UP/MEDIUM；有效产能 UP/HIGH | 升温 | 直接证据 | InferenceXv3在推理过程中实现95%以上KVCache命中率；截至2026年7月底，全国智算总规模达245万PFLOPS(FP16)，其中145万PFLOPS纳入国家级监测调度平台；该节点归属 5 条链，本链语境未确定。对“算力供给”环节而言，这意味着产能利用率上升、商业化进度上升、市场供给上升、性价比上升、有效产能上升，因此本期判断为升温。 | EVTe565f6f7, EVT2c59c6d9 | 中期 | 低–中 |
| `676b3781-e62…` | 模型训练服务 | 市场需求 UP/LOW | 升温 | 推理假设 | 生成式AI基础模型商业化加快，会带来持续训练、微调和迭代需求。模型训练服务是基础模型的明确投入环节，因此推测其市场需求上升。 | 起点证据：EVTea2f0e49 | 中期（传导滞后） | 低（0.60） |
| `f2bddc77-184…` | 生成式AI应用服务 | 商业化进度 UP/LOW | 升温 | 推理假设 | 基础模型商业化加快会提高可用模型供给，并降低下游应用的开发与部署门槛。基础模型是生成式AI应用服务的明确投入，因此推测应用服务的商业化进度上升。 | 起点证据：EVTea2f0e49 | 中期（传导滞后） | 低（0.48） |
...
| `64c968f0-6ec…` | 企业应用软件 | 商业化进度 UP/LOW | 升温 | 推理假设 | AI智能体商业化进度上升，会使企业应用软件更快集成自动执行、知识检索和流程协同能力。企业应用软件明确依赖AI智能体，因此推测其商业化进度上升。 | 起点证据：EVT34119384 | 中期（传导滞后） | 低（0.42） |
| `3ab77588-d8a…` | 企业知识库 | 市场需求 UP/LOW | 升温 | 推理假设 | AI智能体进入商业应用后，需要可检索、可更新且权限可控的企业知识。AI智能体明确依赖企业知识库，因此推测企业知识库的市场需求上升。 | 起点证据：EVT34119384 | 中期（传导滞后） | 低（0.45） |
...
| `36285326-b10…` | 智能终端语音交互应用 | 商业化进度 UP/LOW | 升温 | 推理假设 | 智能语音SDK与云API商业化加快，会降低终端应用集成语音能力的成本和时间。该能力是智能终端语音交互应用的明确投入，因此推测应用端商业化进度上升。 | 起点证据：EVTcff99921 | 中期（传导滞后） | 低（0.48） |
| `ca61bc3c-6dd…` | 智能语音基础模型训练服务 | 市场需求 UP/LOW | 升温 | 推理假设 | 智能语音基础模型的技术成熟度和商业化进度上升，通常需要持续训练、评测和场景微调。训练服务是基础模型的明确投入环节，因此推测其市场需求上升。 | 起点证据：EVTcff99921 | 中期（传导滞后） | 低（0.57） |
...
| `e7447dcf-1a2…` | 换电服务 | 商业化进度 UP/LOW | 升温 | 推理假设 | 标准化换电电池包需求上升，同时换电运营调度平台商业化提速，表明资产与运营系统都在准备扩张。换电服务同时依赖这两个环节，因此推测其商业化进度上升。 | 起点证据：EVT00eaab71 | 中期（传导滞后） | 低（0.45） |
...
| `3089bafe-844…` | 稀释制冷机 | 市场需求 UP/LOW | 升温 | 推理假设 | 超导量子计算机获得更强政策支持后，整机项目投入可能带动关键配套设备。稀释制冷机是超导量子计算机的明确组成环节，因此推测其采购需求上升。 | 起点证据：EVT6ee33536 | 长期（传导滞后） | 低（0.44） |
| `9fb21fae-e19…` | 超导量子芯片 | 市场需求 UP/LOW | 升温 | 推理假设 | 超导量子计算机获得更强政策支持后，研发和项目投入通常会向核心部件传导。超导量子芯片是整机的明确组成环节，因此推测其研发与采购需求上升。 | 起点证据：EVT6ee33536 | 长期（传导滞后） | 低（0.44） |
| `5de95b2e-4f0…` | 量子测控电子系统 | 市场需求 UP/LOW | 升温 | 推理假设 | 超导量子计算机获得更强政策支持后，整机研发和部署将增加对测控环节的需求。量子测控电子系统是整机的明确组成环节，因此推测其市场需求上升。 | 起点证据：EVT6ee33536 | 长期（传导滞后） | 低（0.44） |
...
| `e9af2997-9a9…` | 激光雷达 | 市场需求 UP/LOW | 升温 | 推理假设 | 自动驾驶系统的商业化、订单可见度和技术成熟度同时上升，会增加对环境感知部件的配套需求。激光雷达是系统的真实组成环节，因此推测其市场需求上升。 | 起点证据：EVT39d201dc, EVT8e5d52ba, EVTba2fe6be | 中期–长期（传导滞后） | 中（0.69） |
| `8516fc1c-1d5…` | 车载摄像头模组 | 市场需求 UP/LOW | 升温 | 推理假设 | 自动驾驶系统的商业化、订单可见度和技术成熟度同时上升，会增加多视角环境感知需求。车载摄像头模组是系统的真实组成环节，因此推测其市场需求上升。 | 起点证据：EVT39d201dc, EVT8e5d52ba, EVTba2fe6be | 中期–长期（传导滞后） | 中（0.69） |
...
| `50922f37-966…` | 新能源汽车公告认证服务 | 市场需求 UP/LOW | 升温 | 推理假设 | 纯电动汽车整车的订单和扩产走强，通常伴随新车型、配置和生产变更的准入需求。整车明确依赖公告认证服务，因此推测该服务需求上升，但需后续车型申报数验证。 | 起点证据：EVT2b8388a3, EVT8e5d52ba | 中期（传导假设） | 低（0.48） |
| `8c11adf0-00a…` | 纯电动汽车动力电池包 | 市场需求 UP/LOW | 升温 | 推理假设 | 纯电动汽车整车的订单可见度、扩产强度和渗透率上升，会按整车产量拉动动力电池包配套。动力电池包是整车的明确组成环节，因此推测其市场需求上升。 | 起点证据：EVT2b8388a3, EVT8e5d52ba | 中期（传导滞后） | 中（0.69） |
| `0a94a6fe-97d…` | 纯电动汽车底盘车身系统 | 市场需求 UP/LOW | 升温 | 推理假设 | 纯电动汽车整车的订单可见度和扩产强度上升，会按整车产量增加底盘车身系统的配套量。该系统是整车的明确组成环节，因此推测其市场需求上升。 | 起点证据：EVT2b8388a3, EVT8e5d52ba | 中期（传导滞后） | 中（0.69） |
| `f979b6fa-f2c…` | 纯电动汽车热管理系统 | 市场需求 UP/LOW | 升温 | 推理假设 | 纯电动汽车整车订单和扩产走强，会增加电池、电驱和座舱热管理的配套量。热管理系统是整车的明确组成环节，因此推测其市场需求上升。 | 起点证据：EVT2b8388a3, EVT8e5d52ba | 中期（传导滞后） | 中（0.69） |
| `f320a7ca-8dc…` | 纯电动汽车电驱总成 | 市场需求 UP/LOW | 升温 | 推理假设 | 纯电动汽车整车订单和扩产走强，会按整车产量增加电驱总成的配套量。电驱总成是整车的明确组成环节，因此推测其市场需求上升。 | 起点证据：EVT2b8388a3, EVT8e5d52ba | 中期（传导滞后） | 中（0.69） |
...
| `334fe51e-677…` | 普钢长材轧制工序 | 产能利用率 UP/LOW | 升温 | 推理假设 | 钢铁长材的销售价格、定价权和成本传导能力上升，可能支撑国内长材生产和轧制活动。长材轧制工序是钢铁长材的明确投入环节，因此推测其产能利用率上升。 | 起点证据：EVTd6821525, EVTd2af646a | 短期–中期（传导假设） | 低（0.48） |
...
| `db3e28c5-9ec…` | 成品油批发交付服务 | 投入成本 UP/LOW；交付周期 UP/LOW | 降温 | 推理假设 | 油品运输的交付周期和销售价格上升，会把更高的物流成本和更长的到货时间传导给批发交付环节。成品油批发服务明确依赖运输服务，因此推测其投入成本和交付周期上升，经营景气受压。 | 起点证据：EVTa8b7eff4, EVT3cf927a6 | 短期–中期（传导滞后） | 低（0.62） |
...
| `f33d7edb-720…` | 火电设备 | 市场需求 UP/LOW；扩产强度 UP/LOW | 升温 | 推理假设 | 汽轮机的市场需求和扩产强度上升，表明火电和燃机设备的核心动力环节正在扩张。汽轮机是火电设备的明确组成环节，因此推测整体设备需求和扩产强度上升。 | 起点证据：EVT7cfe2fe7 | 中期–长期（传导假设） | 低（0.48） |
...
| `1ec80957-268…` | 建筑钢筋精整加工服务 | 投入成本 UP/LOW | 降温 | 推理假设 | 热轧带肋钢筋的销售价格和成本传导能力上升，会提高精整加工环节的原料成本。热轧带肋钢筋是建筑钢筋精整加工的明确投入，因此推测该服务的投入成本上升，利润空间可能受压。 | 起点证据：EVTca5d1d59 | 短期（传导滞后） | 低（0.62） |
...
| `454506dd-557…` | 云计算基础设施 | 资本开支 UP/LOW；扩产强度 UP/LOW | 升温 | 推理假设 | 数据中心网络设备需求上升，通常对应云基础设施的新建、扩容或升级投入。网络设备是云计算基础设施的真实组成环节，因此推测云基础设施的资本开支和扩产强度上升。 | 起点证据：EVT40bb2d15 | 长期（传导假设） | 低（0.48） |
...
| `8c89232c-93c…` | NAND Flash封装测试服务 | 市场需求 UP/LOW | 升温 | 推理假设 | NAND Flash存储芯片需求上升，会增加芯片出货前的封装与测试工作量。封装测试服务是NAND Flash芯片的明确投入环节，因此推测其市场需求上升。 | 起点证据：EVT40bb2d15 | 长期（传导滞后） | 低（0.54） |
...
| `3ea1e01f-981…` | eVTOL飞行器 | 技术成熟度 UP/LOW；商业化进度 UP/LOW | 升温 | 推理假设 | eVTOL推进电池系统的性价比上升，有助于缓解飞行器在重量、续航和整机成本上的约束。推进电池是飞行器的明确组成环节，因此推测整机技术成熟度和商业化进度上升。 | 起点证据：EVTc7d71a07 | 中期–长期（传导假设） | 低（0.42） |
...
| `aa623e5b-21c…` | 基因数据分析服务 | 市场需求 UP/LOW | 升温 | 推理假设 | 分子伴随诊断服务商业化加快，会增加对基因数据处理、变异解读和报告生成的需求。基因数据分析是伴随诊断的明确投入环节，因此推测其市场需求上升。 | 起点证据：EVTff5eaf10 | 长期（传导滞后） | 低（0.45） |
| `0946d917-6df…` | 精准用药决策服务 | 商业化进度 UP/LOW | 升温 | 推理假设 | 分子伴随诊断服务商业化加快，会提高可用的患者分层和用药匹配信息供给。伴随诊断是精准用药决策的明确投入，因此推测决策服务的商业化进度上升。 | 起点证据：EVTff5eaf10 | 长期（传导滞后） | 低（0.42） |
...
| X-GEO-MAC-01 | 海湾冲突与霍尔木兹通行受阻增强全球增长预期下修的跨层解释 | EVT3cf927a6, EVTec2c2162, EVTa8b7eff4；目标直接证据 EVTcaf3f60b | 能源替代、运输恢复和政策对冲 | 连续油运、能源价格与增长预测 |
| X-GEO-CHN-01 | 海湾冲突通过油品运输服务向成品油批发交付服务传导 | EVTa8b7eff4, EVT3cf927a6 | 运输恢复或运价回落 | 批发交付节点的采购成本、到货周期与经营数据 |
...
8. 跨层路径只有在目标为真实下层锚点且机制闭合时才进入结论；同一 Event 在两层分别形成直接 Signal 时保留“同源信号”标记，不改写为跨层直接因果。
