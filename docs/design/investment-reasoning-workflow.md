...
输入必须同时包含地缘政治层和宏观经济层结果，以及产业链/节点直接 Signal。

该 Step 才执行产业链检索与拓扑加载：

1. 根据直接产业链 Signal、上层结论及机制 Fact 召回候选 `IndustryChain`；
...
语义相关只能用于候选召回，不能单独启动方向判断。链级 Signal 如果没有可验证的节点落点，也不能直接传播。交叉出现在多条产业链中的节点必须按 `(industry_chain_id, chain_node_id)` 分析，禁止跨链污染。
