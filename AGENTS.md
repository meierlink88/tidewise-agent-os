...
├── Evidence Reviewer (single-article relevance + Evidence; seeded by agents/title_curator.py)
...
├── Event Identity      (Agno Studio/PostgreSQL component; seeded by agents/event_identity.py)
├── Event Signal Analyst (Agno Studio/PostgreSQL component; seeded by agents/event_signal_analyst.py)
├── Investment Reasoner / Report Writer / Reviewer (Studio components; seeded by agents/investment_*.py)
...
- Evidence Reviewer 与 Raw Collection 是显式混合组件：Function 与运行合同由 Git 管理，当前发布的 Reviewer Agent 提示词和 Workflow 编排由 Agno Studio 在 PostgreSQL 中版本化；每次运行加载当前发布版。
- Raw Collection 固定四个 Step：`evidence_collect` → `process_articles` Loop（`prepare_evidence_review` → Evidence Reviewer Agent → `evidence_publish`）。Loop 在 Workflow 中配置，条件封装在 Function 中，Agent 直接配置在 Step 中。单篇准备、审阅或发布失败须持久记录异常、跳过该篇剩余动作并继续下一篇，不自动恢复/补发；无法建立队列或安全持久记录异常的基础设施故障仍终止运行。
- Raw Collection 逐篇审阅并提取 Evidence，逐篇发布 Raw/Evidence 后才进入下一篇。无关或无有效 Evidence 的原文仅保留在本地排除队列；文章身份、发布重放与历史迁移见 `docs/design/article-review-workflow.md`。
