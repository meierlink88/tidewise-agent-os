...
- Raw Collection 固定四个 Step：`evidence_collect` → `process_articles` Loop（`prepare_evidence_review` → Evidence Reviewer Agent → `evidence_publish`）。Loop 在 Workflow 中配置，条件封装在 Function 中，Agent 直接配置在 Step 中。发布失败记录异常并终止运行，不配置自动恢复/补发。
